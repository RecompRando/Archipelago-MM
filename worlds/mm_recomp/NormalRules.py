...
def can_get_magic_beans(state, player):
    return (state.has("Magic Bean", player) and
            (
                state.can_reach("Deku Palace", 'Region', player) or
                (
...
def can_get_cow_milk(state, player):
    return (
        ...
        (
            ...
            state.has("Romani's Mask", player) or
            (
                state.can_reach("Beneath the Well", 'Region', player) and
...
def has_bottle(state, player, need_count=1):
    bottle_count = 0
    if state.has("Bottle", player, 3):
        bottle_count += 3
    elif state.has("Bottle", player, 2):
        bottle_count += 2
        ...
        "Woodfall Temple -> Odolwa's Lair":
            lambda state: (
                ...
                (
                    ...
                    state.has("Fierce Deity Mask", player)
                ) and (
...
def get_location_rules(player, options, prices):
    return {
        ...
        "Southern Swamp Deku Scrub Purchase":
            lambda state: (
                state.has("Magic Beans", player) and
                (
                    state.has("Deku Mask", player)
                ) or 
                (
        ...
        "Swamp Spider House Reward":
            lambda state: (
                (
                    state.has("Swamp Skulltula Token", player, options.required_skull_tokens.value)
                ) or (
                    options.skullsanity.value == 2 and
                    can_smack(state, player) and
                    has_bottle(state, player) and
                    has_projectiles(state, player) and
                    (
        ...
        "Ocean Spider House Reward":
            lambda state: (
                (
                    state.has("Ocean Skulltula Token", player, options.required_skull_tokens.value)
                ) or (
                    options.skullsanity.value == 2 and
                    has_explosives(state, player) and
                    state.has("Hookshot", player) and
        ...
        "Stone Tower Great Fairy Reward":
            lambda state: state.has("Stray Fairy (Stone Tower)", player, options.required_stray_fairies.value),
        ...
        "Moon Link Trial HP":
            lambda state: (
                state.can_reach("Moon Link Trial Iron Knuckle Chest", 'Location', player) and
                has_bombchus(state, player) and
                state.has("Progressive Bow", player)
            ),
